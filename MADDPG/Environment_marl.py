...
class Environ:
    ...
    def add_new_vehicles_by_number(self, n):
        ...
        for i in range(n):
            ...
            self.add_new_vehicles(start_position, start_direction, np.random.randint(5, 10))
    ...
    def Compute_Performance_Reward_Train(self, actions_power):
        actions = actions_power[:, :, 0].astype(int)  # the channel_selection_part    # 离散的，原始code
        ...
        for i in range(len(self.vehicles)):
            for j in range(self.n_neighbor):
                if not self.active_links[i, j]:
                    continue
                # V2I_Interference[actions[i][j]] += 10 ** ((self.V2V_power_dB_List[power_selection[i, j]] - self.V2I_channels_with_fastfading[i, actions[i, j]] + self.vehAntGain + self.bsAntGain - self.bsNoiseFigure) / 10)
                V2I_Interference[actions[i][j]] += power_selection[i, j] * 10 ** ((30 - self.V2I_channels_with_fastfading[i, actions[i, j]] + self.vehAntGain + self.bsAntGain - self.bsNoiseFigure) / 10)

                # V2I_Interference[actions[i][j]] += 10 ** ((power_selection[i, j] - self.V2I_channels_with_fastfading[i, actions[i, j]] + self.vehAntGain + self.bsAntGain - self.bsNoiseFigure) / 10)
        ...
        for i in range(self.n_RB):  # scanning all bands 遍历每一个RB，从actions找到共用一个RB的车号
            ...
            for j in range(len(indexes)):
                ...
                V2V_Signal[indexes[j, 0], indexes[j, 1]] = power_selection[indexes[j, 0], indexes[j, 1]] * 10 ** ((30 - self.V2V_channels_with_fastfading[indexes[j][0], receiver_j, i] + 2 * self.vehAntGain - self.vehNoiseFigure) / 10)
                ...
                for k in range(j + 1, len(indexes)):  # spectrum-sharing V2Vs
                    ...
                    V2V_Interference[indexes[j, 0], indexes[j, 1]] += power_selection[indexes[k, 0], indexes[k, 1]] * 10 ** ((30 - self.V2V_channels_with_fastfading[indexes[k][0]][receiver_j][
                                                                                  i] + 2 * self.vehAntGain - self.vehNoiseFigure) / 10)
                    V2V_Interference[indexes[k, 0], indexes[k, 1]] += power_selection[indexes[j, 0], indexes[j, 1]] * 10 ** ((30 - self.V2V_channels_with_fastfading[indexes[j][0]][receiver_k][
                                                                                  i] + 2 * self.vehAntGain - self.vehNoiseFigure) / 10)
        ...
        self.demand -= V2V_Rate * self.time_fast * self.bandwidth       # 需要的bit数
