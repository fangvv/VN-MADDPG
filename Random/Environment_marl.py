...
class Environ:
    ...
    # 计算Reward：Compute_Performance_Reward_Train(self, actions_power)，这里的输入非常重要，是RL的action，
    # 其定义在main_marl_train.py中，是个三维数组，以（层，行，列）进行说明，一层一个车，一行一个邻居，
    # 共有两列分别为RB选择（用RB的序号表示）和power选择（也用序号表示，作为power_db_list的索引）
